    ...
    switch (d.__typename) {
      case "UserMessage":
        ...
        return {
          ...
          // New
          reactions: d.message_reactions ? d.message_reactions.map(formatReactionsGraphQL) : null,
...
module.exports = function (defaultFuncs, api, ctx) {
  return function getThreadHistoryGraphQL(threadID, amount, timestamp, callback) {
    ...
    defaultFuncs
      ...
      .catch(function (err) {
        log.error("getThreadHistoryGraphQL", "Rate limit reached. (getThreadHistoryGraphQL)");
        return callback(err);
      });
