...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
	...
	ctx.mqttClient = new mqtt.Client(
		() => websocket(host, options.wsOptions),
		options,
	);
			...
			if (v.delta.attachments && i == v.delta.attachments.length) {
				...
				if (
					...
					api.resolvePhotoUrl(
						...
						(err, url) => {
							if (!err)
								v.delta.attachments[i].mercury.metadata.url =
									url;
				...
				if (
					...
					(function () {
						globalCallback(null, {
							...
							reaction:
								delta.deltaMessageReaction.reaction ?? null,
							senderID:
								delta.deltaMessageReaction.senderId.toString(),
							userID: delta.deltaMessageReaction.userId.toString(),
							action: delta.deltaMessageReaction.reaction
								? 'Add'
								: 'Remove',
						});
					})();
					...
					(function () {
						globalCallback(null, {
							...
							senderID:
								delta.deltaRecallMessageData.senderID.toString(),
							deletionTimestamp:
					...
					if (delta.deltaMessageReply.repliedToMessage) {
						...
						callbackToReturn.messageReply = {
							...
							attachments:
								delta.deltaMessageReply.repliedToMessage.attachments
									.map(function (att) {
										var mercury = JSON.parse(
											att.mercuryJSON,
										);
										Object.assign(att, mercury);
										return att;
						...
						return defaultFuncs
							...
							.then((resData) => {
								...
								callbackToReturn.messageReply = {
									...
									attachments:
										fetchData.message.blob_attachment.map(
											(att) => {
												var x;
												try {
		...
		defaultFuncs
			...
			.then((resData) => {
				if (utils.getType(resData) != 'Array')
					throw {
						error: "Can't Sign In - Appstate error",
						res: resData,
					};
			...
			.catch((err) => {
				...
				if (
					...
					err.error === "Can't Sign In - Appstate error"
				)
					ctx.loggedIn = false;
